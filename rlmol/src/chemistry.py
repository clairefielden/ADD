...
# dict() mapping atom_type to max valence
atom_valences = {atom_type: max(list(periodic_table.GetValenceList(
    atom_type)))
...
def get_fingerprint(molecule, fingerprint_length=2048, fingerprint_radius=3):
    if molecule is None:
...
def get_valid_actions(state, config, reset=False):
    ...
    allowed_ring_sizes = config['allowed_ring_sizes']
    ...
    if allow_no_modification and not reset:
