...
# Set up the path variables for where config files, envs and results will be stored
...
scoring_strategy.update({"reaction_filter": reaction_filter})
...
configuration["parameters"]["curriculum_strategy"] = {
    ...
    "diversity_filter": {
        "name": "NoFilterWithPenalty",         # other options are: "IdenticalTopologicalScaffold",
    ...
    "curriculum_objectives": [{
        # 1st scoring function/curriculum objective is to obtain a high tanimoto similarity to the existing Malaria drugs substructures
...
# set up the Production Objective
